...
class AuthService {
  ...
  static async signup(
    ...
    try {
      ...
      if (existingUser) {
        ...
        await UserRepository.createDefaultFavorites(existingUser, options);
  ...
  static async signin(
    ...
    try {
      ...
      const ischeckAll = await this.checkAll(address);

      if (!ischeckAll) {
        throw new Error400(
          options.language,
          'auth.balancelessthanlimit',
        );
      }
  ...
  static async verifyEmail(token, options) {
    ...
    try {
      let record = await TenantRepository.findById('4215cb3b-c18d-405b-b5b6-b985c2bc26b1', {
        ...options,
        transaction
      });
      ...
      const roles = [Roles.values.custom];
  ...
  static async checkAll(address) {
    try {
      ...
      //       switch (entry.key) {
      //         case "VXNlcl9TdGFrZQ==":
      //           //let User_Stake = entry.value.byte
      //           console.log("Yieldly stake detected but unable to parse");
      //           break;
      //         default:
      //           break;
      //       }
      ...
      if (balance >= minBalance) {
        ...
        console.log("balance less than minimum");
        return false;
      }
